...
public class Email {
    ...
    private Date emailExpirationDate; // 이메일 만료 날짜
    ...
    public boolean isExpired() {
        return new Date().after(this.emailExpirationDate);
    }

    // 이메일 만료 시간을 설정하는 메소드 (10분)
    public void setEmailExpirationDate() {
        long expirationTime = 10 * 60 * 1000; // 10분(밀리초 단위)
        this.emailExpirationDate = new Date(this.emailRequestDate.getTime() + expirationTime);
    }
}
